 ...
 */
int LineDescriptor::OctaveKeyLines(const cv::Mat & image_, ScaleLines &keyLines)
{
	cv::Mat image = image_;
	...
	for(unsigned int octaveCount = 0; octaveCount<numOfOctave_; octaveCount++){
		...
		for (int i = 0; i < edLineVec_[octaveCount]->lines_.numOfLines; ++i) {
			const auto &line = edLineVec_[octaveCount]->lineEndpoints_[i];
			cv::line(show_image, cv::Point2f(line[0], line[1]),cv::Point2f(line[2], line[3]), cv::Scalar(0, 255, 0), 1);
		}
...
int LineDescriptor::OctaveKeyLines(const cv::Mat & image_, ScaleLines &keyLines, LSDDetectorC::LSDOptions opts)
{
	cv::Mat image = image_;
	unsigned int numOfFinalLine = 0;
	...
	for(unsigned int octaveCount = 0; octaveCount<numOfOctave_; octaveCount++){
		...
		cv::imwrite("/home/wz/VO-LOAM/github/output/blur.png", blur);
		double line_ratio = atof(line_cfg["line_ratio"].dump().c_str());
        edLineVec_[octaveCount]->dxImg_.create(blur.rows, blur.cols, CV_16SC1);
		edLineVec_[octaveCount]->dyImg_.create(blur.rows, blur.cols, CV_16SC1 );
		cv::Sobel( image, edLineVec_[octaveCount]->dxImg_, CV_16SC1, 1, 0, 3);
       ...
       cv::Ptr<cv::cv_contribe::LineSegmentDetector> ls;
		...
		cv::cvtColor(show_image, show_image, cv::COLOR_GRAY2BGR);
		// blur.convertTo(show_image, CV_8UC3);
		for (int i = 0; i < octave_lines.size(); ++i) {
			const auto &line = octave_lines[i];
			cv::Scalar color(128 + rand()%128, 128 + rand()%128, 128 + rand()%128);
			cv::line(show_image, cv::Point2f(line(0), line(1)),cv::Point2f(line(2), line(3)), color, 1);
		}
		// cv::imshow("show_image", show_image);
		// cv::waitKey();
		
		lines_lsd.push_back(octave_lines);
		numOfFinalLine += octave_lines.size();
...
////////////////////////////////////
	//Reorganize the detected lines into keyLines
	lineIDInScaleLineVec = lines_lsd[0].size();
	keyLines.clear();
	keyLines.resize(lineIDInScaleLineVec);
  unsigned int tempID;
	float s1,e1,s2,e2;
	bool shouldChange;
	OctaveSingleLine singleLine;
	for(unsigned int  oct_id = 0;oct_id < lines_lsd.size(); ++oct_id){
		for (size_t line_id = 0; line_id < lines_lsd[oct_id].size(); ++line_id) {
		
			lineIDInOctave = line_id;
			octaveID       = oct_id;
			double sx = lines_lsd[oct_id][line_id](0);
			double sy = lines_lsd[oct_id][line_id](1);
			double ex = lines_lsd[oct_id][line_id](2);
			double ey = lines_lsd[oct_id][line_id](3);
			double dx = ex - sx;
			double dy = ey - sy;
			direction = std::atan2(dy, dx);
			singleLine.octaveCount = octaveID;
			singleLine.direction = direction;
			singleLine.lineLength = std::hypot(dx, dy);
			singleLine.salience  = 0;
			...
			if(shouldChange){
				...
				singleLine.ePointInOctaveX = s1;
				singleLine.ePointInOctaveY = s2;
				singleLine.direction = std::atan2(-dy, -dx);
				singleLine.startPointX = tempValue * e1;
				singleLine.startPointY = tempValue * e2;
				singleLine.endPointX   = tempValue * s1;
				singleLine.endPointY   = tempValue * s2;
			...
			// tempID = octaveLines[lineID].lineIDInScaleLineVec;
			keyLines[line_id].push_back(singleLine);
		}
	}
...
int LineDescriptor::ComputeLBD_(ScaleLines &keyLines)
{
	...
	for(short lineIDInScaleVec = 0; lineIDInScaleVec<numOfFinalLine; lineIDInScaleVec++){
		...
		for(short lineIDInSameLine = 0; lineIDInSameLine<sameLineSize; lineIDInSameLine++){
			...
			lengthOfLSP = std::ceil(keyLines[lineIDInScaleVec][lineIDInSameLine].lineLength);
			...
			desVec = pSingleLine->descriptor.data();
			for(short i=0; i<numOfBand_; i++){
				(*desVec++) = *(desVec) * tempM;//desVec[8*i] =  desVec[8*i] * tempM;//(*desVec) = *(desVec++) * tempM;
				(*desVec++) = *(desVec) * tempM;//desVec[8*i+1] =  desVec[8*i+1] * tempM;//(*desVec) = *(desVec++) * tempM;
				(*desVec++) = *(desVec) * tempM;//desVec[8*i+2] =  desVec[8*i+2] * tempM;//(*desVec) = *(desVec++) * tempM;
				(*desVec++) = *(desVec) * tempM;//desVec[8*i+3] =  desVec[8*i+3] * tempM;//(*desVec) = *(desVec++) * tempM;
				(*desVec++) = *(desVec) * tempS;//desVec[8*i+4] =  desVec[8*i+4] * tempS;//(*desVec) = *(desVec++) * tempS;
				(*desVec++) = *(desVec) * tempS;//desVec[8*i+5] =  desVec[8*i+5] * tempS;//(*desVec) = *(desVec++) * tempS;
				(*desVec++) = *(desVec) * tempS;//desVec[8*i+6] =  desVec[8*i+6] * tempS;//(*desVec) = *(desVec++) * tempS;
				(*desVec++) = *(desVec) * tempS;//desVec[8*i+7] =  desVec[8*i+7] * tempS;//(*desVec) = *(desVec++) * tempS;
			}
...
int LineDescriptor::GetLineDescriptor(const cv::Mat & image, ScaleLines & keyLines, LSDDetectorC::LSDOptions opts, int lineMode)
{
    double t = (double)cv::getTickCount();
	bool res = false;
	if (lineMode == 0) 	{
		res = OctaveKeyLines(image, keyLines);
	} else if (lineMode == 1) {
		res = OctaveKeyLines(image, keyLines, opts);
	}
    if(!res){
